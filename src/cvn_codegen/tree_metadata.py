...
def load_tree_model(tree_model_path: Path) -> ElementTree.Element:

    if not isinstance(tree_model_path, Path):
        raise ValueError(f"tree_model_path must be a Path object, got {type(tree_model_path)} instead.")
    if not tree_model_path.is_file():
        raise FileNotFoundError(f"Tree model file not found at path: {tree_model_path}")

    tree = ElementTree.parse(tree_model_path)
    root = tree.getroot()
...
def get_attribute(element: ElementTree.Element, attribute_name: str) -> str | None:
...
def build_tree_path_entry(code: str,tree_cvn_item_code: str | None,tree_property_name: str | None,tree_indicator_name: str | None,tree_value: str | None, xml_path: str,) -> TreePathEntry:
    
    normalized_code = str(code).strip()

    if not normalized_code:
        raise ValueError(f"Code cannot be empty or whitespace only. Received code: '{code}'")
...
def collect_indicator_entries(indicator_element:  ElementTree.Element,current_path_segments: list[str], tree_cvn_item_code: str | None,tree_property_name: str | None,) -> list[TreePathEntry]:
...
def collect_property_entries(property_element:ElementTree.Element,current_path_segments: list[str],tree_cvn_item_code: str | None, ) -> list[TreePathEntry]:
    entries: list[TreePathEntry] = []
...
def extract_tree_entries(root: ElementTree.Element) -> list[TreePathEntry]:
    entries: list[TreePathEntry] = []
    node_element: ElementTree.Element | None = None
    for child in root:
    ...
    if node_element is None:
        raise ValueError(f"Expected 'Node' elements under root, but found '{node_element}'.")
    ...
    for container in node_element:
        ...
        if container_tag == "Version":
            ...
            if tree_cvn_item_code is None:
                break
            if tree_cvn_item_code.strip():
                current_path_segments = ["Node", f"CVNItem[@code='{tree_cvn_item_code.strip()}']",]
...
def index_tree_entries_by_code(tree_entries: list[TreePathEntry]) -> dict[str, tuple[TreePathEntry, ...]]:
    ...
    return {
        code: tuple(entries)
        for code, entries in grouped_entries.items()
    }

def index_tree_entries_by_xml_path(tree_entries: list[TreePathEntry]) -> dict[str, tuple[TreePathEntry, ...]]:
    grouped_entries: dict[str, list[TreePathEntry]] = {}
    for entry in tree_entries:
        grouped_entries.setdefault(entry.xml_path, []).append(entry)
    return {
        xml_path: tuple(entries)
        for xml_path, entries in grouped_entries.items()
    }