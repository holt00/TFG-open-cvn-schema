...
`docs/context/current_status.md`, both frozen/closed).
...
- **Memoria**: **50 pages maximum**. Written using the same strategy as the
  ...
  (session-by-session entries in `docs/context/tfm_current_status.md`, this
...
| --- | --- | --- | --- |
| 0. Cluster + core infra | 1-2 | k3s running; core services deployed | k3s up; MinIO, PostgreSQL, Airflow deployed via Helm; each reachable/smoke-tested; `docs/pipeline/tfm_lakehouse_architecture.md` started; daily entries begin in `docs/context/tfm_current_status.md` |
...
- **Documentation update protocol**: at the end of every TFM issue's work
  ...
  `docs/context/tfm_current_status.md` (never the closed
  `docs/context/current_status.md`), (3) `docs/pipeline/known_limitations.md`
  if a new limitation appeared, (4) `docs/roadmap/tfm_roadmap.md` if roadmap
  state changed (never the closed
  `docs/roadmap/cvn_generation_roadmap.md`), (5) `PROJECT_GUIDE.md` if
...
- **Structural/documentation-map changes**: recorded as a hotfix document
  under `docs/roadmap/hotfixes/`, the same convention used for
  `hotfix-1` through `hotfix-9`.
- **Frozen TFG documents**: never add entries to
  ...
  TFM work is logged exclusively in `docs/context/tfm_current_status.md` and
  `docs/roadmap/tfm_roadmap.md`.
- **Mandatory reading order for any session resuming this work**:
  `AGENTS.md` -> `PROJECT_GUIDE.md` ->
  `docs/context/project_context_index.md` ->
  `docs/context/tfm_current_status.md` -> this epic document -> the current
